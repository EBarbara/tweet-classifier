...
class NeuralNetwork:
    ...
    def evaluate_class(self, pred_labels, class_id):
        result_prediction = []
        filepath = 'results/predictions_reduced.csv'
        print_data(filepath, 'id,tweet,pred,real')
        for pred, real, id, tweet in np.nditer([pred_labels, self.real_labels, self.test_id, self.test_tweets]):           
            print_data(filepath, f'{id},{tweet},{pred},{real}')
